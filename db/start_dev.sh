...
docker volume create $VOLUME_NAME ; docker kill $CONTAINER_NAME ; docker rm $CONTAINER_NAME ; docker network rm $NETWORK_NAME
docker network create $NETWORK_NAME
docker run --name $CONTAINER_NAME --network $NETWORK_NAME -e POSTGRES_PASSWORD=$DEV_PASSWORD -v $VOLUME_NAME:/var/lib/postgresql/data:rw -d postgres:11
